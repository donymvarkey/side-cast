...
const DeviceDetailsDialog = ({
  ...
  const renderBatteryStatus = (status: string) => {
    switch (status) {
      case "Full":
        return (
          <BatteryFull className={"w-5 h-5 fill-green-500 text-green-500 "} />
        );
      case "Charging":
        return (
          <BatteryCharging
      ...
      case "Discharging":
        return (
          <BatteryMedium className={"w-5 h-5 fill-amber-500 text-amber-500"} />
        );
      case "Not charging":
        return (
          <BatteryWarning className={"w-5 h-5 fill-red-500 text-red-500"} />
        );
      case "Unknown":
      ...
      <DialogContent className={"bg-gray-900 text-gray-100 border-0 min-w-2/3"}>
        <DialogHeader>
          <DialogTitle className={"flex items-center space-x-2"}>
            ...
            <Badge
              className={
                "bg-green-500 text-white font-quicksand-medium uppercase ml-6"
              }
            >
              {device?.connection}
            </Badge>
          ...
          <DialogDescription className={"mt-5 grid grid-cols-4 gap-4"}>
            <div className={"flex items-center gap-x-2"}>
              ...
              <div>
                <span
                  className={"text-gray-500 font-quicksand-regular text-xs"}
                >
                  Serial
                </span>
                <p className={"font-quicksand-semibold text-gray-100"}>
                  {device?.serial}
                </p>
              </div>
            </div>
            <div className={"flex items-center gap-x-2"}>
              ...
              <div>
                ...
                <p
                  className={"font-quicksand-semibold text-gray-100 capitalize"}
                >
                  {device?.brand}
                </p>
              </div>
