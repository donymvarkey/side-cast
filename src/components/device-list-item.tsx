...
const DeviceListItem = ({serial, model, product, device, status, mirrorDevice, getInfo,  onOpenChange}: {serial: string, model: string, product: string, device: string, status: string, mirrorDevice: (serial: string) => void, getInfo: (serial: string) => void, onOpenChange: () => void}) => {
		...
		<Card key={serial} className="bg-gray-900 text-white border border-gray-700 ">
			...
			<CardHeader className="flex flex-row items-center justify-between space-y-0">
				<CardTitle className="text-md font-quicksand-semibold flex items-center gap-2">
					...
					{model}
					<div className={cn("size-3  rounded-full", status === "device" ? "bg-green-500" : "bg-red-500")} />
				</CardTitle>
			...
			<CardContent className="space-y-1 font-quicksand-regular text-sm">
				...
				<div className="flex gap-2 pt-2 items-center justify-between">
					<Button className={"bg-blue-500 hover:bg-blue-600 flex-1"}  onClick={() => mirrorDevice(serial)}>
						<Monitor className="w-4 h-4 mr-1" /> Mirror
					</Button>
					<Button onClick={() => handleDeviceInfo(serial)} variant="secondary" className={"flex-1"} >
