...
const Home = () => {
  ...
  const [runningInstances, setRunningInstances] = useState({}); // Track running instances of scrcpy

  const getAdbServerState = async () => {
    window.electronAPI.invoke("adb:get-server-state").then((state: boolean) => setIsServerRunning(state));
  }
  ...
  const handleMirror = async (serial: string) => {
    if(Object.keys(runningInstances) && runningInstances[serial]) {
      toast("Device " + serial + " is already being mirrored.");
      return;
    }
    const res = await window.electronAPI.invoke("adb:mirror-device", serial, []);
    setRunningInstances({...runningInstances, [serial]: true})
    if (!res.success) {
  ...
  return (
    <div className={"w-full h-full"}>
      <div className={"flex flex-col w-full"}>
        <div className={"flex items-center justify-end self-end gap-x-2 w-full mr-10"}>
          <div className={"flex items-center gap-x-2"}>
            <div className={cn("rounded-full size-2 animate-pulse", isServerRunning ? "bg-green-500" : "bg-red-500")} />
            <div className={"text-gray-400 font-quicksand-medium text-xs"}>ADB Server status: <span className={"font-quicksand-semibold text-gray-300"}>{isServerRunning ? "Active": "Stopped"}</span></div>
          </div>
          <Button className={"bg-gray-100 text-gray-900"} onClick={handleRestartServer}>
            <RefreshCw />
          </Button>
          {
            isServerRunning ? <Button className={"bg-red-500 text-white"} onClick={handleStopServer}>
              <PowerOff />
            </Button> : <Button className={"bg-green-500 text-white"} onClick={handleStartServer}>
              <Power />
            </Button>
          }
        </div>
        <h1 className={"text-gray-400 font-quicksand-semibold mb-4 mt-2"}>Connected Devices</h1>
        {loading ? (
      ...
      {
        showDetailsDialog && <DeviceDetailsDialog device={deviceDetails} open={showDetailsDialog} onOpenChange={() => setShowDetailsDialog((prev:boolean) => !prev)} />
      }
    </div>
  );
};
